...
function generateGuardBotJs(d, ci) {
  ...
  ln("app.listen(PORT,async function(){console.log('Guard bot starting on port '+PORT);await new Promise(function(r){setTimeout(r,2000);});await registerWebhook();setInterval(function(){if(WEBHOOK_URL){fetch(WEBHOOK_URL+'/health').catch(function(){});}},4*60*1000);console.log('"+TICKER+" guard bot is live');});");
...
function generateFullBotJs(d, ci) {
  ...
  ln("async function checkSpam(ctx,uid){var now=Date.now(),t=spamTracker.get(uid)||{count:0,since:now};if(now-t.since>SPAM_WINDOW)t={count:0,since:now};t.count++;spamTracker.set(uid,t);if(t.count>SPAM_MAX){try{await ctx.telegram.restrictChatMember(ctx.chat.id,uid,{permissions:{can_send_messages:false},until_date:Math.floor(Date.now()/1000)+300});}catch(_){}var m=await ctx.reply('Muted 5 min for spam.');autoDelete(ctx.chat.id,m.message_id,15000);return true;}return false;}");
  ln("var notLiveMsgs=['"+TICKER+" hasn\\u2019t launched yet. CA drops soon.','Not yet. Stay patient.','CA isn\\u2019t live yet. You\\u2019ll know first.','Hold tight. Launches soon.','Sit tight \\u2014 launch is close.'];");
  ln("var caPrompts=['2 short punchy lines about why "+TICKER+" is the move. No CA.','2 lines. "+TICKER+" fundamentals solid. Renounced, locked LP, active dev. No CA.','2 lines. "+TICKER+" found early feeling. No CA.','2 lines. "+TICKER+" built different. Safety, community, momentum. No CA.','2 lines. "+TICKER+" holders are early. No CA.','2 lines. "+TICKER+" worth attention now. No CA.','2 lines. "+TICKER+" moving. Get in now. No CA.'];");
  ...
  ln("var xPrompts=['1 short punchy line about "+TICKER+" on Twitter. Real energy. No hashtags.','1 line. "+TICKER+" X is where the alpha drops.','1 tweet-energy line. Follow "+TICKER+" on X.','1 line. Why following "+TICKER+" on X is smart.','1 line. "+TICKER+" Twitter updates worth watching.'];");
  ln("var xPromptIdx=0;");
  ...
  ln("var silenceAngles=['2-3 bullish lines. Why buy and hold "+TICKER+". Calm confidence.','2-3 lines. Early to "+TICKER+". DOGE/PEPE comparison.','2-3 lines. "+TICKER+" fundamentals: renounced, locked LP, low tax.','2-3 lines. Psychology of early "+TICKER+" investors.','2-3 lines. "+TICKER+" community strength.','2-3 lines. "+TICKER+" different from the noise.','2-3 lines. "+TICKER+" clean project. Just building.','2-3 lines. Quiet before the pump. "+TICKER+" holders know something.'];");
  ln("var silenceIdx=0;");
  ...
  ln("process.on('uncaughtException',function(e){console.error('Uncaught:',e.message);});");
  ln("process.on('unhandledRejection',function(e){console.error('Rejection:',e&&e.message);});");
  ln("app.listen(PORT,async function(){console.log('Bot starting on port '+PORT);await new Promise(function(r){setTimeout(r,2000);});await registerWebhook();resetSilence();setInterval(function(){if(WEBHOOK_URL){fetch(WEBHOOK_URL+'/health').catch(function(){});}},4*60*1000);console.log('"+TICKER+" bot is live');});");
