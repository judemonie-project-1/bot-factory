...
function newSession() {
  return {
    step: 'chain',
    data: {
      chain: 'bsc', tokenName: '', ticker: '', ca: '', supply: '',
      maxWalletPct: '', maxWalletTokens: '', buyTax: '', sellTax: '',
      twitter: '', website: '', renounced: '', locked: '',
      narrative: '', botToken: '', revealCmd: 'revealca', hideCmd: 'hideca',
    },
    imageBuffer: null,
  };
}
...
var STEPS = [
  { key: 'chain',     ask: '\u{1F7E1} Chain? Reply <b>bsc</b> or <b>sol</b>' },
  { key: 'name',      ask: '\u270F\uFE0F Token name? (e.g. PECKER)' },
  { key: 'ticker',    ask: '\u270F\uFE0F Ticker with $? (e.g. $PECKER)' },
  { key: 'ca',        ask: '\u270F\uFE0F Contract address?' },
  { key: 'supply',    ask: '\u270F\uFE0F Total supply? (e.g. 1,000,000,000)' },
  { key: 'maxwallet', ask: '\u270F\uFE0F Max wallet % / token count? (e.g. 4.9% / 49,000,000)\nSend <b>-</b> to skip' },
  { key: 'taxes',     ask: '\u270F\uFE0F Buy / sell tax? (e.g. 5/5)\nSend <b>-</b> if no tax' },
  { key: 'twitter',   ask: '\u270F\uFE0F Twitter/X link?' },
  { key: 'website',   ask: '\u270F\uFE0F Website? (send <b>-</b> to skip)' },
  { key: 'renounced', ask: '\u270F\uFE0F Contract renounced? (yes/no)' },
  { key: 'locked',    ask: '\u270F\uFE0F LP locked? (yes/no)' },
  { key: 'narrative', ask: '\u270F\uFE0F Token story / narrative?' },
  { key: 'image',     ask: '\u270F\uFE0F Send the bot image now (JPG or PNG)' },
  { key: 'bottoken',  ask: '\u270F\uFE0F BotFather token for this new bot?' },
  { key: 'revealcmd', ask: '\u270F\uFE0F Secret reveal-CA command? (send <b>-</b> for default: revealca)' },
  { key: 'hidecmd',   ask: '\u270F\uFE0F Secret hide-CA command? (send <b>-</b> for default: hideca)' },
];
...
function buildSummary(s) {
  ...
  return (
    ...
    '<b>Image:</b> ' + (s.imageBuffer ? E.check + ' received' : E.warn + ' missing') + '\n\n' +
...
bot.command('help', function(ctx) {
  return ctx.reply(
    ...
    '/help \u2014 This message\n\n' +
    '<b>Supported chains:</b> BSC \u2022 Solana',
    { parse_mode: 'HTML' }
  );
});
...
function handleImageBuffer(ctx, buf) {
  var uid = String(ctx.from.id);
  var s = sessions[uid];
  if (!s || s.step !== 'image') return ctx.reply('No active build waiting for image. Use /build first.');
  s.imageBuffer = buf;
...
async function runBuild(ctx, s, uid) {
  ...
  if (!groqKey) return ctx.reply(E.xmark + ' No Groq key. Use /addgroq KEY first.');
  ...
  try {
    ...
    var botCode = generateBotJs(d, ci, svcUrl);
    ...
    await ctx.reply(E.xmark + ' Build failed:\n<code>' + (err.message || '').slice(0, 400) + '</code>\n\nSend /build to retry.', { parse_mode: 'HTML' });
    delete sessions[uid];
  }
}
...
async function renderCreateService(name, ghOwner, ownerId) {
  var r = await fetch('https://api.render.com/v1/services', {
    ...
    body: JSON.stringify({
      ...
      serviceDetails: { buildCommand: 'npm install', startCommand: 'npm start', plan: 'free', region: 'oregon', runtime: 'node', numInstances: 1 },
    }),
  });
  var d = await r.json();
  var svc = d.service || d;
...
function generatePackageJson(tokenName) {
  return JSON.stringify({
    name: tokenName.toLowerCase().replace(/[^a-z0-9]/g, '-') + '-bot', version: '1.0.0', main: 'bot.js',
...
function generateBotJs(d, ci, webhookUrl) {
  ...
  ln("  return 'You are the official Telegram bot for "+TICKER+", a "+CHAIN_LBL+" meme token.\\n\\n'+");
  ln("    'TOKEN FACTS:\\n'+");
  ln("    '- Name: "+NAME+" | Ticker: "+TICKER+"\\n'+");
  ln("    '- Blockchain: "+CHAIN_LBL+"\\n'+");
  ln("    '- Total Supply: "+SUPPLY+"\\n'+");
  ln("    '- Max Wallet: "+MAX_PCT+(MAX_TOK?" / "+MAX_TOK+" tokens":"")+"\\n'+");
  ln("    '- Buy Tax: "+BUY_TAX+"% | Sell Tax: "+SELL_TAX+"%\\n'+");
  ln("    '- Contract: "+RENOUNCED+" \\u2014 permanently locked. Security feature.\\n'+");
  ln("    '- Liquidity: "+LOCKED+" \\u2014 funds fully secured.\\n'+");
  ln("    '- DEV STATUS: Dev is ACTIVE. NEVER imply dev stepped back or is gone.\\n'+");
  ln("    '- Twitter/X: '+TWITTER+'\\n'+");
  ln("    (withCa?'- CA: '+CA+'\\n':'')+");
  ln("    (withCa?'- Chart: '+CHART+'\\n':'')+");
  ln("    (withCa?'- Buy on "+DEX_NAME+": '+BUY+'\\n':'')+");
  ln("    '\\nNARRATIVE:\\n'+" + NARR + "+'\\n\\n'+");
  ln("    'PERSONALITY: Calm, confident, warm, genuinely bullish. Every reply different. Never robotic. Minimal emojis. Short questions=1-3 lines, detailed=up to 5.\\n\\n'+");
  ln("    'HARD RULES: NEVER share TG group link. NEVER volunteer CA. NEVER emoji same line as CA. NEVER repeat reply. If hype/casual chat/no answer needed: reply IGNORE';");
  ln("}");
